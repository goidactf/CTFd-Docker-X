...
class DockerChallengeTracker(db.Model):
    ...
    """
    id = db.Column(db.Integer, primary_key=True)
    team_id = db.Column("team_id", db.String(128), index=True)
    user_id = db.Column("user_id", db.String(128), index=True)
...
def create_container(docker, image, team, portbl):
    ...
    container_name = "%s_%s" % (image.replace('/', '_').replace(':', '_'), team)
    assigned_ports = dict()
...
class ContainerAPI(Resource):
    ...
    def get(self):
        ...
        return {'revert_seconds': revert_seconds}
